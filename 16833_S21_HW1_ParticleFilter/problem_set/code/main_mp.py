...
def motion_sensor(x_t0, u_t0, u_t1, ranges, motion_model,sensor_model, meas_type):
    x_t0_wt = x_t0[3]
    x_t0 = x_t0[:3]
...
if __name__ == '__main__':
    ...
    for time_idx, line in enumerate(logfile):
        ...
        args_mc = [[X_bar[m], u_t0, u_t1, ranges, motion_model,sensor_model, meas_type] for m in range(0, num_particles)]

        with Pool(num_workers) as p:
            X_new = p.starmap(motion_sensor, args_mc)

        X_bar = np.asarray(X_new)
        u_t0 = u_t1
        """
        RESAMPLING
        """
