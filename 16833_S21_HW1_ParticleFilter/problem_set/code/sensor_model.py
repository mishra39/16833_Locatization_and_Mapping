...
class SensorModel:
    ...
    def beam_range_finder_model(self, z_t1_arr, x_t1):
        ...
        for k in range(0,k_tot):
            ...
            if ((z_t1_arr[k] >= 0) and (z_t1_arr[k] <= self._max_range)):
                p_hit = np.random.normal(loc=z_star_k, scale=self._sigma_hit, size=None)
            else:
                p_hit = 0
            
            if ((z_t1_arr[k] >= 0) and (z_t1_arr[k] <= z_star_k)):
                nu = 1 / (1 - math.exp(-self._lambda_short*z_star_k))
                p_short = nu * self._lambda_short * math.exp(-self._lambda_short*z_t1_arr[k])
            else:
                p_short = 0
            
            if (z_t1_arr[k] == self._max_range):
                p_max = 1
            else:
                p_max = 0
