...
class SensorModel:
    ...
    def __init__(self, occupancy_map):
        ...
        self._sigma_hit = 10 # Changed from 50
        self._lambda_short = 0.02 # Changed from 0.1
    ...
    def rayCasting(self, x_t1, z_k, laser_theta):
        ...
        # angles
        for i in range(8000):
            angle = math.pi/2 - self._subsampling*i
            if (self._map[map_x][map_y] and max(x_curr,y_curr) <= 8000 and min(x_curr,y_curr) >=0):
                x_curr += step_x*math.cos(angle)
                y_curr += step_y*math.sin(angle)
                map_x = math.floor(x_curr/10)
                map_y = math.floor(y_curr/10)
